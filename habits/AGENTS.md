...
| `js/assistant-*.js` | — | Optional local Qwen3.8 chat, loaded after first home render by `js/assistant-loader.js`. Natural-language routing is **always model-first**: parser output does not choose intent, split clauses, populate the model envelope, merge tool arguments, or recover from a failed model call. Deterministic helpers validate tool schemas, normalize dates/clocks, read authoritative app data, and perform confirmed writes. The first model pass exposes final validated tools, so `classify_intent` is optional rather than a mandatory round trip. Context entry points (add-sheet sparkles button, detail "change with AI" row) pass `startIntent`/`draft` via `assistantOpenWithInstruction` so the turn starts at extract; trace provenance is `entry-add`/`entry-detail`. `draft_item` creates/changes item settings; `draft_setting` handles weather profiles, places, busy times, and topics; `draft_batch` handles new lists/pastes. `apply_items` changes, snoozes, shows, or deletes a queried set (topic, name, conditions, or the previous list), including different patches for different subsets. Read-only tools cover weather/schedule (`answer_weather`, `answer_schedule`), task/habit lists and progress (`answer_items`), saved setting lists (`answer_settings`), and per-item summary/history/stats/planner explanations (`lookup_item`). `answer_weather` query `hours` ranks a window (several conditions, or lowest/highest of one metric); query `compare` scores two times, including an offset from sunset; query `item` with a start checks a habit in that window even when it is not already planned. `answer_schedule` query `missed` is the same list as the today-header missed pill; day/list payloads include priority and frequency. Compound questions, follow-ups, and several asks in one message share one LLM loop: after a read, `draft_item`, `draft_setting`, `apply_items`, or complete/plan/delete, Tings returns the tool payload and gives the model another turn. A lone `draft_batch` already holds every new item, so that call publishes the preview; sibling tools in the same response still run. The model decides whether a part remains; the harness does not infer that from phrasing. That follow-up is capped at two, and the whole turn at four model calls (six for a multi-item turn). Past the cap, staged work is kept and the model is not asked again. Sibling `tool_calls` all run, including after a no-op, and multiple writes retain separate confirmation actions. Focusing a saved item for complete, plan, delete, or lookup is pronoun context, not a staged edit, so a model stop still returns those confirmations or the tool answer. A `draft_setting` emitted beside a `prepare_action` read stays staged and does not end the turn, because that setting has not seen the result. The model then calls whichever action the user asked for; it does not invent a task or habit. Repeating the same `draft_setting` finishes a request whose action is the setting itself. One confirmation saves every staged draft. The next user turn sees `recent.items` / `recent.referent` in the envelope so "that one" can `lookup_item` and "those" repeats the same read with sort and position instead of querying every saved habit. Explicit names never fall back to a focused or recent item; only omitted names and pronouns may use context. `find_item` ranks saved titles by deterministic fuzzy match; a unique hit is used, and several close hits ask the user instead of guessing. `ask_user` (and classify `unclear`) ask one short question when the request is confusing — at most twice in a row, then Tings stops instead of guessing. A whole-week reschedule, or deleting every saved item with no topic, name, or filter, is `classify_intent` `unsupported`, not a clarifying question. Changing, snoozing, or deleting a matching set is `apply_items`. `complete_item` can log minutes/value/note or undo today's latest completion; `plan_item` adds, replaces, or removes one-day plans; and `delete_item` removes an item. Every action creates a confirmation preview and uses the normal recoverable undo toast. Direct-object setting patches ("add the location home") stay edit-like even with a create verb, while descriptive weather clauses inside a create ("add a Stretch … using Dry weather") never reclassify it as an edit — keep `assistantLooksLikeEdit`'s three-way order (object patch → create guard → heuristics) intact |
