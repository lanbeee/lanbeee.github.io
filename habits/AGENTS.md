...
| `js/assistant-*.js` | — | Optional local Qwen3.8 chat, loaded after first home render by `js/assistant-loader.js`. A residue-audited fast path (`assistantFastPathRisk`) answers only provably-simple phrasing locally (with **use AI instead**). Untrusted parses and that retry withhold `extractedFacts` and do not merge parser fields into `draft_item`. Generation budgets leave thousands of thinking tokens before the tool call. Context entry points (add-sheet sparkles button, detail "change with AI" row) pass `startIntent`/`draft` via `assistantOpenWithInstruction` so the turn starts at extract without the classify call; trace provenance is `entry-add`/`entry-detail`, and model trace rows carry per-call ms + token counts. The model’s validated `draft_item` calls create or change any mix of item settings; `draft_setting` creates weather profiles, places, busy times, and topics. A weather-profile follow-up `weatherText` (for example "prefer higher temperature") merges onto `currentDraft.rules` and must not wipe other metrics. A habit/task `weatherText` with no covering profile materializes one on save. Unknown `placeNames` on a single `draft_item` keep `currentDraft` and ask; the next line is extract/`draft_item`, not classify. Several items or a long paste skip the local parser: the model calls `draft_batch` (not a calendar regex). Unknown places become placeholder locations with dummy addresses (user refines later), TBA rows with no days and no times are dropped, and the preview saves all rows at once |
