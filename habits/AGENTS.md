...
| `js/assistant-*.js` | — | Local Qwen3.8 chat (128k context, compact at 85%): a residue-audited fast path (`assistantFastPathRisk`) answers only provably-simple phrasing locally (with **use AI instead**). Untrusted parses and that retry withhold `extractedFacts` and do not merge parser fields into `draft_item`. Generation budgets leave thousands of thinking tokens before the tool call. The model’s validated `draft_item` calls create or change any mix of item settings; `draft_setting` creates weather profiles, places, busy times, and topics. A habit/task `weatherText` with no covering profile materializes one on save. Unknown `placeNames` keep `currentDraft` and ask; the next line is extract/`draft_item`, not classify |
