...
**Query tools** answer questions by computing against live data; the model selects the tool and fields but never invents the result. `answer_weather` reads day/hour forecasts and assesses weather against an item's rules. `answer_schedule` reads open time, contiguous openings, the freest day, what-if conflicts, the same missed list as the today-header pill, and day/week agendas (day rows include priority and frequency). Compound questions, follow-ups, and several asks in one message use the same loop: after any read tool (or a complete/plan/delete preview) Tings returns the payload and the model gets another turn to call the next tool or answer from that data — ranking phrases are not required. Several `tool_calls` in one model reply all run before that turn. The next user message includes `recent.items` / `recent.referent` so “when did I last do that one?” can `lookup_item` without guessing. A ranking question such as “most important missed” or “most frequent on tomorrow’s agenda” fetches that list first, then may call `lookup_item` for extra history/stats before answering from the tool data. `answer_items` lists tasks/habits by kind and open/done/overdue status or summarizes today's progress. `answer_settings` lists saved places, weather profiles, topics, and busy times. `lookup_item` reports an item's current state and important settings, recent log details, pace/streak/progress statistics, or a concrete planner explanation for the next seven days. Its default summary searches the complete rendered week plus explicit future plan/due markers and reports both the next occurrence and latest real completion, so “not today” is not treated as a complete answer. Spoken names are ranked against the full saved list (fragments, nicknames, typos, or the whole question); a unique hit is used, and several close hits ask which item it is instead of guessing or creating a new one. `find_item` exposes that same ranking. When the whole request is confusing, Tings asks one short question (optional chips) instead of guessing — at most two clarification questions in a row, then it stops. A focused follow-up question uses `lookup_item` again rather than being misread as an edit. Where data is missing or a question is vague, the reply says so honestly. Query tools are read-only: no draft, no save.
