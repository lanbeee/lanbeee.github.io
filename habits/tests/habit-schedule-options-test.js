...
(async()=>{
  ...
  await dynamicRow.locator('.time-endpoint[data-field="start"] .time-mode-toggle').click();
  ...
  await dynamicRow.locator('.time-endpoint[data-field="end"] .time-mode-toggle').click();
  ...
  assert(await page.locator('#detail-habit-option-add').textContent() === 'add','option action adds another specific row');
